...
2.  **Claim Extraction**: The `sub` (Subject) claim of the JWT MUST be treated as the `tenantId`.
...
4.  **Application Injection**: The extracted `tenantId` MUST be injected into the Command or Query as a mandatory argument.
